$(document).ready(function () {
     ...
     * USER MANAGEMENT                          *
     ********************************************/
    // Use delegated handlers so all buttons in the list work (buttons are repeated per user)
    ...
    $(document).on('click', '.btn-ban, .btn-delete, .btn-unban', function () {
        var button = $(this);
        let dataAttr = button.data('attr');
        let userId = dataAttr.userId;
        ...
        let confirmMsg = '';
        if (button.hasClass('btn-ban')) {
            confirmMsg = 'Bạn có chắc chắn muốn chặn người dùng này?';
        } else if (button.hasClass('btn-delete')) {
            confirmMsg = 'Bạn có chắc chắn muốn xóa người dùng này?\n\nNgười dùng sẽ chỉ được xóa nếu không liên kết với bất kỳ dữ liệu nào. Nếu có liên kết, người dùng sẽ bị chặn thay vì xóa.';
        } else if (button.hasClass('btn-unban')) {
            confirmMsg = 'Bạn có chắc chắn muốn bỏ chặn người dùng này?';
        }
            ...
            .done(function (response) {
                if (response.success) {
                    if (button.hasClass('btn-delete')) {
                        ...
                        button.closest('.profile_details').fadeOut(300, function () { $(this).remove(); });
                        toastr.success(response.message);
                        return;
                    }

                    // Update status text and toggle buttons
