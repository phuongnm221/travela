$(document).ready(function () {
    ...
    // reCAPTCHA control flags (exposed on window so modal callback can set them)
    window.captchaInProgress = false; // modal currently asking for captcha
    window.submissionFromCaptcha = false; // submit triggered by captcha callback
                ...
                success: function (response) {
                    if (response.success) {
                        ...
                        if (response.unactivated) {
                            ...
                            $("#resend-activation-link").show();
                            // Lưu username để dùng cho resend activation
                            sessionStorage.setItem('unactivated_username', userName);
                        } else {
                            toastr.error(response.message);
                            // Ẩn link resend nếu lỗi khác
                            $("#resend-activation-link").hide();
                            sessionStorage.removeItem('unactivated_username');
                        }
                    }
                },
    ...
    $("#register-form").on("submit", function (e) {
        ...
        if (!fullname) {
            isValid = false;
            $("#validate_fullname_regis").show().text("Vui lòng nhập họ và tên.");
        }
        ...
        var userName = email;
        $("#username_register").val(userName);
    ...
    $("#forgot-form").on("submit", function (e) {
        e.preventDefault();
        var emailOrUsername = $("#email_forgot").val().trim();

        $("#validate_email_forgot").hide().text("");

        if (!emailOrUsername) {
            $("#validate_email_forgot")
                .show()
    ...
    $("#reset-form").on("submit", function (e) {
        e.preventDefault();
        var password = $("#password_reset").val().trim();
        var passwordConfirm = $("#password_confirm").val().trim();

        $("#validate_password_reset").hide().text("");
        $("#validate_confirm_reset").hide().text("");
...
$("#end_date").datetimepicker({
    ...
    // <-- Thêm chỗ này ngay sau hàm
    $("#agree").on("change", toggleButtonState);


    // TÌM CÁC ĐOẠN CODE TƯƠNG TỰ VÀ XÓA/SỬA
    
    // Hàm kiểm tra tính hợp lệ của form đặt tour
    ...
    //gan gia tri thanh toan vao input hidden de luu vao db
    ...
    $("#comment-form").on("submit", function (e) {
        ...
        $.ajax({
            url: urlSubmitReview, // route lưu review
            method: "POST",
            data: {
                tourId: tourIdReview,
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    $("#partials_reviews .comment-body").addClass("aos-animate");
                    ...
                    // Backend trả lỗi nghiệp vụ (chưa đặt tour / đã review)
                    toastr.error(response.message);
                }
            },
            error: function (xhr, status, error) {
    ...
    $("#resend-activation-link").on("click", function (e) {
        e.preventDefault();
        var username = sessionStorage.getItem('unactivated_username') || $("#username_login").val().trim();
        
        if (!username) {
            toastr.error('Vui lòng nhập tên đăng nhập hoặc email của bạn!');
            return;
        }

        $.ajax({
            type: "POST",
            url: "{{ route('resend-activation') }}",
            data: {
